...
# is ozone level related to temperature?
ns[, .(temperature, ozone)] %>% cor; # Ozone hole is in Antartica!
...
# Aggregate
ns[, .(.N, min_temp = min(temperature), mean_pres = mean(pressure))];
...
# special symbols: .N, .I, .SD, .SDcols
ns[, lapply(.SD, mean), by = year];
ns[, lapply(.SD, mean), .SDcols = c("ozone", "pressure", "surftemp", "temperature"), by = year];
ns[, c(num_obs = .N, lapply(.SD, mean, na.rm = T)), by = year]
# first 2 entries by each year?
ns[, .I[1:2], year];
ns[, .SD[1:2], year];
# max temperature by year?
ns[, .SD[which.max(temperature)], by = year];
# assign a group name
ns[, grp := .GRP, by = .(year, month)];
# assign a group count to each group
ns[, grp_cnt := 1:len(.I), by = .(year, month)];
...
ns[!(month %between% c(3,10)), c(num_obs = .N, lapply(.SD, mean, na.rm = T)), .SDcols = cloudhigh:temperature, by = year]; # winter months
...
ns[, year_mon_lag := lag(year_mon, 1), by = .(lat, long)];
ns[, date_lag := lag(date, 1), by = .(lat, long)];
